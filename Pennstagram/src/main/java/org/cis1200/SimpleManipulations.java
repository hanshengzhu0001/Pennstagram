...
public class SimpleManipulations {
    ...
    public static PixelPicture rotateCCW(PixelPicture pic) {
        ...
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[w][h]; // w is the number of columns, h is the number of rows

        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                tgt[w - col - 1][row] = src[row][col]; // row is y, col is x
            }
        }
        return new PixelPicture(tgt);
        /*
         * the PixelPicture constructor takes in a bitmap,
         * a 2D pixel array to create an PixelPicture object
         */
    }
    ...
    public static PixelPicture border(
        ...
        for (int col = 0; col < w + 2 * borderWidth; col++) {
            for (int row = 0; row < h + 2 * borderWidth; row++) {
                //iterate over tgt
                if (col < borderWidth || row < borderWidth || col >= w + borderWidth
                        || row >= h + borderWidth) {
                    tgt[row][col] = borderColor;
                } else {
                    tgt[row][col] = src[row - borderWidth][col - borderWidth];// translate
                }
            }
        }
     ...
     * algorithm. Gray = (red+green+blue)/3
     *
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];/*
                                         * p and bmp[row][col] point to the same references (a
                                         * pixel),
                                         * however updating p means p points to a new object, so bmp
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int r = Math.min(Math.max((int) Math.round(p.getRed() * rfactor),0),255);
                int g = Math.min(Math.max((int) Math.round(p.getGreen() * gfactor),0),255);
                int b = Math.min(Math.max((int) Math.round(p.getBlue() * bfactor),0),255);
                bmp[row][col] = new Pixel(r, g, b);
            }
        }
        return new PixelPicture(bmp);
